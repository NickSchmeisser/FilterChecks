...
test_ref_rate = ref_file[1]/(28811.29+28669.57)
print(test_ref_rate)
...
for i in range(len(run_numbers)):
    summary_dir = out_dir+str(run_years[i])+"/mono/"+str(run_numbers[i])
    if(os.path.isdir(summary_dir)):
        ...
        if(rates[1][-1]>0.7 or rates[1][-1]<0.4):
           print(run_numbers[i])
j=0
for i in range(len(run_numbers)):
    summary_dir = out_dir+str(run_years[i])+"/mono/"+str(run_numbers[i])
...
#ax0.set_yscale("log")
fig.tight_layout()
fig.savefig("./plots_mono/mono_rates.png", bbox_inches='tight')
plt.close(fig)
...
ax0.set_yscale("log")
fig.tight_layout()
fig.savefig("./plots_mono/mono_rates_pvalues.png", bbox_inches='tight')
plt.close(fig)
...
#ax0.set_yscale("log")
fig.tight_layout()
fig.savefig("./plots_mono/mono_rates_zoom.png", bbox_inches='tight')
plt.close(fig)
...
#ax0.set_yscale("log")
fig.tight_layout()
fig.savefig("./plots_mono/mono_rates_hist.png", bbox_inches='tight')
plt.close(fig)
...
#ax0.set_yscale("log")
fig.tight_layout()
fig.savefig("./plots_mono/mono_rates_hist_zoom.png", bbox_inches='tight')
plt.close(fig)
