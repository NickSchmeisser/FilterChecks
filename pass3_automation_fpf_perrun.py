...
import faint_particle_filter_validation
...
for i in range(len(run_dirs)):
    if(run_years[i] in years):
        ...
        faint_particle_filter_validation.main(in_dir=run_dirs[i], out_dir = directory_name, run_num=run_numbers[i])
        print("Run: "+str(run_numbers[i])+" done!")
'''
for i in years:
    directory_name = out_dir + str(i)+"/fpf/"
