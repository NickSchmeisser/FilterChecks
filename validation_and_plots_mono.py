            ...
            for run in runs:
                ...
                with h5py.File(file_path, 'r') as hf:
                    ...
                    if(len(data>=1)):
                        ...
                        p_values[0].append(years[j])
                        p_values[1].append(pvalue)
                    if labels[i] is None and 'label' in hf[f'var{i}'].attrs:
                        label = hf[f'var{i}'].attrs['label']
                        if isinstance(label, bytes):
                            label = label.decode('utf-8')
                        labels[i] = label
    #return p_values
    #Plotting p_values for variables
        var_name = labels[i]
        fig = plt.figure(figsize=(7, 5))
        ax0=plt.subplot2grid((1,1), (0, 0))
        year_array = [int(i) for i in p_values[0]]
        ax0.plot(year_array,p_values[1],linestyle='None',markersize=7, marker='.')
        ax0.set_xlabel("Years", fontsize=15)
        ax0.set_ylabel("p_value", fontsize=15)
        ax0.set_title(labels[i], fontsize=15)
        ax0.grid(True, linewidth=0.5)
        ax0.set_yscale("log")
        fig.tight_layout()
        fig.savefig(os.path.join(output_dir, f"{var_name}_pvalues_single.png"), bbox_inches='tight')
        plt.close(fig)
        if((np.array(p_values[1])<1e-20).any()):
            slice = np.array(p_values[1])>1e-20
            fig = plt.figure(figsize=(7, 5))
            ax0=plt.subplot2grid((1,1), (0, 0))
            year_array = [int(i) for i in p_values[0]]
            ax0.plot(np.array(year_array)[slice],np.array(p_values[1])[slice],linestyle='None',markersize=7, marker='.')
            ax0.set_xlabel("Years", fontsize=15)
            ax0.set_ylabel("p_value", fontsize=15)
            ax0.set_title(labels[i], fontsize=15)
            ax0.grid(True, linewidth=0.5)
            ax0.set_yscale("log")
            fig.tight_layout()
            fig.savefig(os.path.join(output_dir, f"{var_name}_pvalues_single_zoomed.png"), bbox_inches='tight')
            plt.close(fig)
        print(f"Variable {var_name} done!")
...
def plot_distributions(combined_years, combined_ref, labels, years, output_dir, normalize=False):
    ...
    for i, var_name in enumerate(labels):
        ...
        for j in range(len(combined_years)):
            ...
            ax1.set_ylabel(f"reference/year", fontsize=14)
